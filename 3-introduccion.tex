...
En este contexto, el objetivo general de este trabajo es desarrollar y evaluar un sistema de control semafórico basado en aprendizaje por refuerzo multiagente para mejorar la operación del tránsito en redes urbanas. En particular, la investigación compara el desempeño de algoritmos de entrenamiento centralizado con ejecución descentralizada (\acrshort{acr-mappo} y \acrshort{acr-happo}) con esquemas de aprendizaje independiente (\acrshort{acr-ippo} e \acrshort{acr-idqn}) bajo condiciones controladas, con el fin de evaluar estrategias de coordinación cooperativa y de tratamiento de la heterogeneidad entre intersecciones. La evaluación empírica se lleva a cabo mediante simulaciones microscópicas en escenarios sintéticos de complejidad creciente y en la red vial modelada del microcentro de la Ciudad de Mendoza, utilizando el control de tiempo fijo como ancla operacional de referencia.

La propuesta incorpora una codificación de la espera acumulada por carril mediante una transformación logarítmico-lineal en $I$ niveles. La resolución no uniforme distingue con mayor detalle valores bajos de esa medida y acota los valores representados a partir de un umbral. La elección de cómo representar el tránsito influye en el problema de aprendizaje y cuenta con antecedentes en control semafórico \cite{genders2018representations}. En este trabajo, la codificación se utiliza en todos los controladores.

También se estudia una adaptación de \acrshort{acr-happo} para intersecciones con distintas geometrías, fases disponibles y demandas. A diferencia de la actualización de actores utilizada en \acrshort{acr-mappo}, esta adaptación los actualiza secuencialmente y repondera las ventajas individuales según los cambios de política precedentes \cite{kuba2021trust}. El crítico centralizado incorpora información conjunta durante el entrenamiento. La comparación con \acrshort{acr-mappo}, \acrshort{acr-ippo} y \acrshort{acr-idqn} examina el desempeño de procedimientos completos en esos escenarios, sin atribuir de antemano una ventaja a HAPPO ni aislar el efecto de su actualización secuencial.
...
Para dar respuesta a los objetivos planteados y estructurar el desarrollo de la investigación, el presente documento se organiza en cinco capítulos y un apéndice técnico.

El presente capítulo contextualiza la problemática de la congestión vehicular urbana, las limitaciones de los esquemas de control tradicionales y las oportunidades del aprendizaje por refuerzo adaptativo, estableciendo la motivación, el alcance y los objetivos que guían la investigación. A continuación, el capítulo \ref{cap:marco_teorico} presenta los fundamentos teóricos indispensables para el desarrollo del trabajo, integrando los conceptos centrales de la ingeniería de tránsito, la formalización de procesos de decisión de Markov, los principios del aprendizaje profundo y los paradigmas de aprendizaje por refuerzo multiagente, con especial atención al esquema de entrenamiento centralizado con ejecución descentralizada y al tratamiento de la heterogeneidad entre agentes.

La formulación metodológica y su validación empírica se estructuran en los capítulos siguientes. El capítulo \ref{capitulo:metodologia} detalla el diseño de la investigación: define la representación de observaciones, acciones y funciones de recompensa, formaliza los algoritmos de control comparados y describe la construcción de los entornos de simulación microscópica en \gls{acr-sumo} —incluyendo redes sintéticas y la red del microcentro de la Ciudad de Mendoza calibrada mediante el estadístico \acrshort{acr-geh}—, junto con los protocolos experimentales de entrenamiento y evaluación. Luego, el capítulo \ref{capitulo:resultados} analiza y discute los resultados obtenidos en las simulaciones, contrastando el desempeño operacional de las políticas aprendidas frente al control de tiempo fijo de referencia en términos de demoras, colas y estabilidad durante el entrenamiento.

Finalmente, el capítulo \ref{conclusiones} sintetiza las conclusiones principales a la luz de los objetivos propuestos, delimita el alcance y las limitaciones del estudio, y proyecta líneas de trabajo futuro. Como complemento técnico, el apéndice \ref{apendice:arquitectura_software} expone la arquitectura del framework \texttt{marlTLC}, documentando su diseño orientado a objetos mediante diagramas de clases en \gls{acr-uml} y los patrones aplicados para modularizar la interacción entre la simulación del tránsito y los algoritmos de aprendizaje.
