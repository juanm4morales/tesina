...
Este capítulo describe el diseño experimental, la operacionalización de las variables, la construcción de los escenarios de simulación y la arquitectura computacional utilizada. También presenta los protocolos de entrenamiento y evaluación empleados para comparar \acrshort{acr-mappo} y \acrshort{acr-happo} con las líneas de base \acrshort{acr-ippo} e \acrshort{acr-idqn}. El estudio adopta un enfoque cuantitativo y experimental de medidas repetidas: cada ejecución de entrenamiento produce una política y cada política se evalúa bajo semillas y condiciones de demanda previamente definidas. La simulación microscópica del tránsito funciona como un entorno controlado; el escenario de la Ciudad de Mendoza se interpreta como una construcción sintética restringida por datos parciales, no como un gemelo digital completo.

\section{Enfoque y Diseño de Investigación}
\label{sec:enfoque_diseno}

Este trabajo adopta un \textbf{diseño experimental cuantitativo de medidas repetidas}. Los factores principales son la familia algorítmica, la arquitectura de la red neuronal, el grado de cooperación vecinal $\rho_{\mathrm{coop}}$ y el escenario de tránsito. Las políticas se comparan dentro de cada escenario y, de forma complementaria, se estudia cómo cambia su comportamiento al aumentar la escala o modificar la topología. Dado que también cambian la demanda y el número de agentes, las comparaciones entre escenarios se expresan mediante métricas absolutas y mediante variaciones relativas respecto del controlador de tiempo fijo del mismo escenario.
...
Para representar esta información se aplica la clase \texttt{Encoder} de \path{src/marlTLC/utils/encoder.py}. La transformación no lineal comprime el rango de espera y produce índices discretos; esto limita la magnitud de los valores de entrada, aunque no constituye por sí mismo una garantía sobre los gradientes de la red. La implementación combina una parte logarítmica con una corrección lineal:
\begin{equation}
...
\end{equation}
donde $x\geq 0$ es la espera acumulada del carril, expresada en vehículo$\cdot$segundo, $M_{\mathrm{lane}}>0$ es la cota de saturación de ese carril (en la misma unidad), $I\geq 2$ es el número de intervalos discretos y $L=(I-1)/M_{\mathrm{lane}}$ y $F=(1-L)(I-1)/\log_2(1/L+1)$ son coeficientes calculados para esa cota. Para evaluar la expresión, $x$ y $M_{\mathrm{lane}}$ se introducen como valores numéricos en la misma unidad base. La ecuación \eqref{eq:encoder_log_lineal} transforma la espera continua en un índice discreto. La función $\operatorname{ceil}$ redondea hacia arriba al entero más cercano. La implementación satura la salida: cuando $x$ alcanza la cota $M_{\mathrm{lane}}$ devuelve directamente el índice máximo $I-1$ y, para valores menores, limita el resultado de la expresión al intervalo $[0,I-1]$; el valor que recibe la red neuronal pertenece siempre a ese rango discreto. La letra $M_{\mathrm{lane}}$ se utiliza aquí exclusivamente para el codificador y no debe confundirse con el factor de ventaja $M$ de HAPPO.
...
\begin{enumerate}
    \item \textbf{Etapa 1 (exploración bayesiana amplia):} muestreo mediante \gls{acr-tpe} sobre un presupuesto de 100 pruebas por combinación, con un tope de 150 establecido en el protocolo antes de observar los resultados. ASHA aplica poda temprana con período de gracia de 10 iteraciones y factor de reducción 4; las pruebas podadas no integran el conjunto de candidatos finales. Se evalúan las arquitecturas compacta (\texttt{h128}: $[128, 128]$) y ancha (\texttt{h256}: $[256, 256]$) como estudios independientes de Ray Tune. El espacio de búsqueda comprende 13 dimensiones para \acrshort{acr-mappo} e \acrshort{acr-ippo}, y 14 para \acrshort{acr-happo}:
    \begin{itemize}
...
En síntesis, el diseño estima el cambio de la espera media por vehículo insertado entre cada política aprendida y el control de tiempo fijo del mismo escenario, con la ejecución completa de entrenamiento como unidad experimental y con CR, IR, CI, teletransportes y equidad espacial como controles de atención a la demanda y de estabilidad. Sus principales restricciones de validez externa son el carácter restringido por datos del escenario de la Ciudad de Mendoza, la línea base de tiempo fijo generada por \acrshort{acr-sumo} ---que no reconstruye los planes semafóricos municipales--- y el uso de una misma familia de recompensa local-vecinal en todos los algoritmos. El Capítulo \ref{capitulo:resultados} presenta los resultados obtenidos con este protocolo.
