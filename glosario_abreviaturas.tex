...
\newacronym{acr-marl}{MARL}{Aprendizaje por refuerzo multiagente (\textit{Multi-Agent Reinforcement Learning})}
\newacronym{acr-madrl}{MADRL}{Aprendizaje por refuerzo profundo multiagente (\textit{Multi-Agent Deep Reinforcement Learning})}
...
\newacronym{acr-dqn}{DQN}{Red Q profunda (\textit{Deep Q-Network})}
\newacronym{acr-idqn}{IDQN}{Red Q profunda independiente (\textit{Independent Deep Q-Network})}
...
\newacronym{acr-gae}{GAE}{Estimación generalizada de ventaja (\textit{Generalized Advantage Estimation})}
\newacronym{acr-trpo}{TRPO}{Optimización de política por región de confianza (\textit{Trust Region Policy Optimization})}
\newacronym{acr-ppo}{PPO}{Optimización de política proximal (\textit{Proximal Policy Optimization})}
\newacronym{acr-ippo}{IPPO}{Optimización de política proximal independiente (\textit{Independent Proximal Policy Optimization})}
\newacronym{acr-mappo}{MAPPO}{Optimización de política proximal multiagente (\textit{Multi-Agent Proximal Policy Optimization})}
\newacronym{acr-happo}{HAPPO}{Optimización de política proximal con agentes heterogéneos (\textit{Heterogeneous-Agent Proximal Policy Optimization})}
\newacronym{acr-hatrpo}{HATRPO}{Optimización de política por región de confianza con agentes heterogéneos (\textit{Heterogeneous-Agent Trust Region Policy Optimization})}
\newacronym{acr-a2c}{A2C}{Actor--crítico con ventaja (\textit{Advantage Actor-Critic})}
\newacronym{acr-ddpg}{DDPG}{Gradiente de política determinista profundo (\textit{Deep Deterministic Policy Gradient})}
\newacronym{acr-maddpg}{MADDPG}{Gradiente de política determinista profundo multiagente (\textit{Multi-Agent Deep Deterministic Policy Gradient})}
\newacronym{acr-qmix}{QMIX}{Red de mezcla de valores Q (\textit{Q-Mixing Network})}
\newacronym{acr-vdn}{VDN}{Redes de descomposición de valor (\textit{Value-Decomposition Networks})}
\newacronym{acr-wolf-phc}{WoLF-PHC}{Ascenso de colina en la política con ganar o aprender rápido (\textit{Win or Learn Fast Policy Hill-Climbing})}
...
\newacronym{acr-phemlight}{PHEMlight}{Modelo de emisiones para vehículos livianos y pesados (\textit{Passenger car and Heavy duty Emission Model light})}
\newacronym{acr-hpo}{HPO}{Optimización de hiperparámetros (\textit{Hyperparameter Optimization})}
\newacronym{acr-tpe}{TPE}{Estimador de Parzen estructurado en árbol (\textit{Tree-structured Parzen Estimator})}
\newacronym{acr-asha}{ASHA}{Algoritmo asíncrono de reducción sucesiva a la mitad (\textit{Asynchronous Successive Halving Algorithm})}
...
\newacronym{acr-slurm}{SLURM}{Herramienta simple de Linux para gestión de recursos (\textit{Simple Linux Utility for Resource Management})}
...
\newacronym{acr-cr}{CR}{Tasa de completitud (\textit{Completion Rate})}
\newacronym{acr-ir}{IR}{Tasa de inserción (\textit{Insertion Rate})}
\newacronym{acr-ci}{CI}{Tasa de completitud de vehículos insertados (\textit{Completion of Inserted Vehicles})}
\newacronym{acr-geh}{GEH}{Estadístico empírico de Geoffrey E. Havers}
...
\newacronym{acr-flir}{FLIR}{Infrarrojo de barrido frontal (\textit{Forward Looking Infrared})}
...
\newacronym{acr-es}{E/S}{Entrada/salida (\textit{Input/Output})}
