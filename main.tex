\documentclass[11pt,a4paper]{report}
\usepackage[margin=2.5cm]{geometry}
\usepackage{afterpage}
\usepackage{titlesec}
\titleformat{\chapter}[display]{\vspace{-3cm}\normalfont\bfseries}{}{0pt}{\LARGE}
\usepackage[provide=*,spanish]{babel}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{color}
\usepackage[table]{xcolor}
\usepackage{graphicx}
\usepackage[export]{adjustbox}
\usepackage{tikz}
\usetikzlibrary{positioning,arrows.meta,calc,shapes.geometric}
\usepackage{pgfplots}
\pgfplotsset{compat=1.18}
\usepackage{amssymb}
\usepackage{url}
\usepackage{xcolor}
\usepackage{caption} 
\usepackage{float}
\usepackage{listingsutf8}
\usepackage[framemethod=default]{mdframed}
\usepackage{needspace}

\usepackage{algorithm}
\usepackage{algorithmicx}
\usepackage{algpseudocode}
%agregado caro para comentarios
\usepackage{todonotes}

% --- Paletas de Colores de la Tesis ---
% 1. Paleta Categórica: Dinámica del Entorno
\definecolor{catVerdeMenta}{HTML}{A8E6CF}
\definecolor{catMelocoton}{HTML}{FFD3B6}
\definecolor{catRosaSalmon}{HTML}{FFAAA5}
\definecolor{catAzulCielo}{HTML}{A4C8F0}
\definecolor{catLilaSuave}{HTML}{CBAACB}

% 2. Paleta Secuencial: Mapas de Calor y Señales
\definecolor{seqNivelUno}{HTML}{F9E076}
\definecolor{seqNivelDos}{HTML}{F3B87A}
\definecolor{seqNivelTres}{HTML}{E28F8E}
\definecolor{seqNivelCuatro}{HTML}{B9749F}
\definecolor{seqNivelCinco}{HTML}{7A5C96}

% 3. Paleta Divergente y Comparativa: Rendimiento de Políticas
\definecolor{divBaseline}{HTML}{D9D9D9}
\definecolor{divAltUno}{HTML}{99D5CA}
\definecolor{divAltDos}{HTML}{A0AEC1}
\definecolor{divPropuesto}{HTML}{F2A68D}
% --------------------------------------

% --- Paleta Semántica por Dominio ---
% Convenciones de uso (NO redefinir los colores, solo usar estos aliases):
%
% Dinámica de Tránsito:
%   colVehiculo       -> vehículos en el simulador microscópico
%   colConflicto      -> puntos de conflicto, corriente saturada / platoon
%   colFlujoLibre     -> flujo libre, exponencial negativa, separaciones temporales
%   colSeparacionEsp  -> líneas guía de separaciones espaciales (e, s)
%   colSeparacionTmp  -> líneas guía de separaciones temporales (g, h)
%
% Señalización Semafórica:
%   colSemaforoRojo    -> fase roja del semáforo
%   colSemaforoAmarillo-> fase amarilla del semáforo
%   colSemaforoVerde   -> fase verde del semáforo
%
% Arquitectura del Sistema (Metodología):
%   colEntorno  -> simulador microscópico (SUMO)
%   colInterfaz -> interfaz de control (TraCI)
%   colAgente   -> política de control / agente DRL
%
% Series de Datos:
%   colDatoPrimario -> serie principal de datos (p. ej. flota vehicular)
%
% Taxonomías y Diagramas Jerárquicos:
%   colTaxonomiaNivelUno   -> categoría raíz o concepto general
%   colTaxonomiaNivelDos   -> categorías intermedias
%   colTaxonomiaNivelTres  -> subcategorías o ramas específicas

\colorlet{colVehiculo}{catAzulCielo!80!black}
\colorlet{colConflicto}{catRosaSalmon!80!black}
\colorlet{colFlujoLibre}{catVerdeMenta!80!black}
\colorlet{colSeparacionEsp}{catRosaSalmon!80!black}
\colorlet{colSeparacionTmp}{catVerdeMenta!80!black}

\colorlet{colSemaforoRojo}{catRosaSalmon!80!white}
\colorlet{colSemaforoAmarillo}{seqNivelUno!80!white}
\colorlet{colSemaforoVerde}{catVerdeMenta!80!white}

\colorlet{colEntorno}{catAzulCielo}
\colorlet{colInterfaz}{catVerdeMenta}
\colorlet{colAgente}{catRosaSalmon}

\colorlet{colDatoPrimario}{catAzulCielo!80!black}

\colorlet{colTaxonomiaNivelUno}{catAzulCielo!40}
\colorlet{colTaxonomiaNivelDos}{catMelocoton!40}
\colorlet{colTaxonomiaNivelTres}{catVerdeMenta!40}
% --------------------------------------

\definecolor{LightGray}{gray}{0.9}
\usepackage[spanish]{babelbib}
\definecolor{codegreen}{rgb}{0,0.6,0}
\definecolor{codegray}{rgb}{0.5,0.5,0.5}
\definecolor{codepurple}{rgb}{0.58,0,0.82}
\definecolor{backcolour}{rgb}{0.95,0.95,0.92}
\lstdefinestyle{codigo}{
    backgroundcolor= \color{backcolour},   
    commentstyle=\color{codegreen},
    keywordstyle=\color{magenta},
    numberstyle=\tiny\color{codegray},
    stringstyle=\color{codepurple},
    basicstyle=\scriptsize\normalfont\sffamily,  
    breakatwhitespace=false,         
    breaklines=true,                 
    captionpos=t,                 
    keepspaces=true,
    numbers=left,                   
    numbersep=5pt,  
    showspaces=false,                
    showstringspaces=false,
    showtabs=false,                  
    tabsize=2,
    frame=single,
    framexleftmargin=15pt,
    framexrightmargin=5pt,
    framexbottommargin=5pt,
    framextopmargin=5pt,
    inputencoding=utf8,
    extendedchars=true,
    literate = {¬}{{$\neg$}}1
}

\DeclareCaptionFormat{listing}{#1#2#3}
\captionsetup[lstlisting]{format=listing,singlelinecheck=false, margin=0pt, font={sf},labelsep=space,labelfont=bf}

%Paquete para compuestos químicos
\usepackage[version=4,arrows=pgf-filled,
textfontname=sffamily,
mathfontname=mathsf]{mhchem} %comando \ce{H2O} para escribir fórmulas químicas

\renewcommand{\lstlistingname}{Código}
\renewcommand{\lstlistlistingname}{Índice de códigos}

\usepackage[many]{tcolorbox}  
\newtcolorbox{boxB}{
    boxrule = 1.5pt,
    colframe = white,
    rounded corners,
    arc = 5pt
}

\definecolor{mygreen}{rgb}{0,0.6,0}
\definecolor{mygray}{rgb}{0.5,0.5,0.5}
\definecolor{mymauve}{rgb}{0.58,0,0.82}
\definecolor{terminalbgcolor}{HTML}{330033}
\definecolor{terminalrulecolor}{HTML}{000099}

\lstdefinestyle{terminal}
{
    backgroundcolor=\color{black},
	basicstyle=\scriptsize\color{white}\ttfamily,
	breaklines=true,
	captionpos=t,
	extendedchars=true,
	showspaces=false,
	showstringspaces=false,
	frame=single,
    framexleftmargin=5pt,
    framexrightmargin=5pt,
    framexbottommargin=5pt,
    framextopmargin=5pt,
    breakindent=0pt,
    breakautoindent=false,
    literate={\$}{{\$}}1 
         {:}{{:}}1
         {¬}{{$\neg$}}1
         {~}{{\textasciitilde}}1,
}

\usepackage{hyperref}
\hypersetup{
colorlinks,
allcolors=black,
linktoc=all
}

%CAROLINA para que sean tablas en vez de cuadros
\setlocalecaption{spanish}{table}{Tabla}
\setlocalecaption{spanish}{listtable}{Índice de tablas}

% comment commands
\newcommand {\JM}[1]{{\color{blue} {\textbf{Juan Martín: #1}}}}
\newcommand {\PJV}[1]{{\color{green} {\textbf{Pablo: #1}}}}
\newcommand {\ACO}[1]{{\color{red} {\textbf{Caro: #1}}}}

\usepackage[nonumberlist]{glossaries}
\loadglsentries{glosario.tex}
\makeglossaries

\begin{document}

\include{0-titulo}
\afterpage{\null\thispagestyle{empty}\newpage}
\include{1-resumen}
\include{1-agradecimientos}

\tableofcontents
\thispagestyle{empty}

\lstlistoflistings
\thispagestyle{empty}

\listoffigures
\thispagestyle{empty}

\glsaddall
\printglossary[title=Glosario,toctitle=Glosario]
\thispagestyle{empty}
\todo[inline]{Hace un glosario de términos y variables para que sea de consulta rápida para el lector. El glosairo debe estar ubicado luego de los índices antes del texto cosa de que si alguien se pierde pueda recurrir rápido a él. Por más que uno sepa del tema luego de leer tantas letras se hace lio. El glosario no es un diccionario, es un listado de términos y variables que se usan en la tesis con una breve explicación de cada uno. Se puede hacer un glosario de términos y otro de variables. Hacerlo ya no esperar al final se hace mientras se escribe porque luego seguro te olvidas de agregar cosas.}
\include{2-resumen}
\include{3-introduccion}
\include{4-marco_teorico}
\include{5-metodologia}
\include{6-resultados}
\include{7-conclusiones}

\bibliographystyle{babalpha}
\bibliography{bibliografia}

\end{document}
