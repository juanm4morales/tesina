...
\begin{itemize}
    \item \textbf{Flujo o tasa de flujo ($q$):} representa la cantidad de vehículos que atraviesan una sección transversal de la vía por unidad de tiempo. Si el período de observación es menor a una hora, se proyecta como una tasa horaria equivalente en vehículos por hora (veh/h), $q = N/T$ \cite{calymayor}.
    \item \textbf{Densidad o concentración ($k$):} indica el número de vehículos que ocupan simultáneamente una longitud específica de vía, expresada en vehículos por kilómetro (veh/km), $k = N/d$ \cite{calymayor}. En la práctica, se suele estimar indirectamente a través del porcentaje de ocupación temporal registrado por algun tipo de detector \cite{ite_handbook}.
...
La interrupción del flujo causada por semáforos mal configurados o saturación genera efectos adversos que los sistemas de control buscan mitigar. Siguiendo la terminología de la ingeniería de tránsito, las principales manifestaciones medibles de esta ineficiencia son las demoras, las colas y las detenciones \cite{fernandez}:
\begin{itemize}
...
Una suposición habitual del MDP es que el agente observa un estado suficientemente informativo del entorno. Sin embargo, en muchas aplicaciones reales el agente solo recibe observaciones parciales o ruidosas. En estos casos, el marco se generaliza a un Proceso de Decisión de Markov Parcialmente Observable (POMDP)\todo[inline]{si esta abrevitura es en inglés agregar lo que significa en inglés (Partially Observable Markov Decision Process)}, donde el historial de observaciones puede ser necesario para inferir el estado subyacente \cite{marlbook}. Esta distinción resulta importante para el control semafórico: una simulación puede poseer un estado global bien definido, pero cada controlador suele operar con mediciones locales, como colas, ocupación o tiempos de espera observados en sus aproximaciones.
...
Los algoritmos de RL pueden agruparse, a grandes rasgos, en métodos basados en valor y métodos basados en política. Esta distinción no agota la taxonomía del campo, pero permite introducir las familias de algoritmos más relevantes para este trabajo \cite{suttonRL}.
...
\todo[inline]{agregar un texto introductorio tiras subsection tambi´pen poner deep learning (aprendizaje profundo) en algun lado y luego subsubsection pelada queda mal}
\subsubsection{La Unidad Neuronal y Redes \textit{Feedforward}}
\todo[inline]{buscar más citas para poner de redes neuronales, poner una sola una y otra vez queda mal más para un tema tan amplio y con tanta historia. \cite{haykin1994neural} \cite{bishop1995neural} o \cite{goodfellow2016deep}}
En problemas de control complejos, los métodos tabulares se vuelven impracticables debido a las restricciones de memoria y cómputo que impone la \todo{de donde sale la palabra maldición acá????? ojo con copiar y pegar traducciones poco felices que hacen los traductores ... pensar en cómo uno define las cosas}maldición de la dimensionalidad \cite{suttonRL}. La aproximación de funciones soluciona esta limitación al representar funciones mediante una forma parametrizada, permitiendo que el aprendizaje en un estado se generalice a otros estados con características similares. El aprendizaje profundo (\textit{Deep Learning}) emplea redes neuronales artificiales como aproximadores de funciones continuas y no lineales \cite{marlbook}.

El componente básico de estas arquitecturas es la unidad neuronal individual\todo[inline]{definir bien qué es una nuerona artificial, poner un dibujo darle un poco de vida al texto porque es muy muy plano}. Computacionalmente, una neurona artificial en una capa $k$ ejecuta dos operaciones secuenciales sobre el vector de características de entrada $\mathbf{x}$ \todo[inline]{queda raro que no se sabe que es un vector de características, mejor no complicarse y poner algo como dada un vector de entrada $\mathbf{x}$ }. Primero, calcula una combinación lineal utilizando un vector de pesos $\mathbf{w}$ y un sesgo (\textit{bias}) escalar $b$. Luego, el resultado se somete a una función de activación no lineal \todo[inline]{no se entiende porque es de activación...recordar que tal vez le interse leer la tesina a un ingenierio civil y lee esto y no entiende nada... fijate que te pones a hablar de capa y luego definis más abajo las capas por ahi te conviene arrancar más ordenado.}$g_k$:
\begin{equation}
    f_{k,u}(\mathbf{x}; \theta_k^u) = g_k(\mathbf{w}^\top \mathbf{x} + b)
\end{equation}
\todo[inline]{agregar un dibujo donde se vea cada uno de los conceptos alrededor de la neurona artificial}
La inclusión de una función de activación es estrictamente necesaria; componer múltiples capas sin funciones no lineales resultaría analíticamente en una simple transformación lineal, anulando la capacidad de representación de la red \cite{marlbook}. Entre las opciones no lineales más comunes destacan ReLU (Unidad Lineal Rectificada, que emite la entrada directamente si es positiva y cero en caso contrario), \textit{leaky} ReLU (una variante que permite un gradiente pequeño para entradas negativas, previniendo la parálisis de la neurona) y Tanh (tangente hiperbólica, que comprime la salida continua en el rango $[-1, 1]$). \todo[inline]{si en algún lugar de tu tesis trabajas con redes nueronales tenes que poner las funciones de activación bien definidas y no nombradas a la ligera porque luego las vas a nombrar. Mirar las citas de redes nueronales que te puse y cómo explican los conceptos para que queden claros.}
...
\todo{buscar en las citas que te deje de NN para ver cómo se explica. Poner un dibujo de red para clarificar conceptos y referenciarla en el texto para ir explicándolos.}


\subsubsection{El Proceso de Optimización}

Dado que las redes neuronales carecen de una solución de forma cerrada, sus parámetros $\theta$ se ajustan utilizando métodos de optimización iterativa. El proceso se fundamenta en tres componentes técnicos \todo{poner más citas de redes nueronales como \cite{haykin1994neural} \cite{bishop1995neural} o \cite{goodfellow2016deep} no usar siempre las mismas.. porque estás introduciendo conceptos.}\cite{marlbook}:
...
\begin{itemize}
    ...
    \item \textbf{\textit{Bootstrapping}:} La práctica de actualizar estimaciones de valor apoyándose en otras estimaciones aprendidas previamente (como en los métodos \emph{Temporal Difference}), en lugar de esperar la consecución de retornos reales completos.
...
En los métodos basados en valor, DRL sustituye la tabla clásica por una red neuronal $Q(s, a; \theta)$. En espacios de acción discretos, el algoritmo \textit{Deep Q-Networks} (DQN) procesa el estado $s$ como vector de entrada y computa simultáneamente las estimaciones de valor para todas las acciones en su capa de salida \cite{marlbook, mnih2015}.\todo{meterle un dibujo para clarificar.}
...
\todo[inline]{poner una explicación minima del Algoritmo \ref{alg:dqn} y nombrarlo porque tirarlo ahí nomas sin decir nada lo deja totalmente descolgado. Todo algoritmo figura o tabla debe estar explicada y refernciada dentro del texto.}
...
\end{equation}
Esta actualización incrementa la probabilidad de ejecutar aquellas acciones que están asociadas a mayores retornos esperados, normalizadas por su probabilidad actual \cite{marlbook}. \todo[inline]{figura explicativa. Si esto lo usas para tu tesis tenes que poner un dibujo de la red neuronal que representa la política y explicar cómo se hace el gradiente de política dentro de RL también fijate que cada cosa que aparece en esa ecuación esté explicada. Acordate de nombrar las ecuaciones en el texto y no poner dos puntos y tirarla nomas ahí... porque si despues la usas más abajo necesitas que tenga un label y póder nombrarla.}

\subsubsection{\textit{Proximal Policy Optimization} (PPO)}

DQN pertenece a la familia de métodos basados en valor y resulta natural para espacios de acción discretos. Sin embargo, una parte importante del DRL moderno utiliza métodos actor-crítico, que combinan aprendizaje directo de políticas con estimación de valores. En este esquema se mantienen dos componentes, implementados como redes separadas o como cabezales de una misma arquitectura \todo{figura explicativa}:
\begin{itemize}
...
\todo{veo muchísimas variables definidas no explicadas en todo el texto revisar y agregar en el texto la explicación de cada una de ellas. Por ejemplo, acá tenes que explicar qué es c1 c2 el $\epsilon$, cada vez agregar más variables sin explicar....}
...
PPO es esencialmente un método \textit{on-policy}: recolecta trayectorias con la política vigente, estima ventajas y realiza varias épocas de optimización sobre ese bloque de experiencia. En el contexto de este trabajo, su importancia no radica solo en su desempeño como algoritmo de agente único, sino también en que sirve como base para extensiones multi-agente utilizadas como líneas de investigación actuales, entre ellas IPPO, MAPPO y HAPPO \todo{abreviaturas sin explicación, algoritmo sin referencia en el texto ni explicado en el texto }.
