...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        cat1/.style={box, fill=catAzulCielo!40, text width=4cm},
        cat2/.style={box, fill=catMelocoton!40, text width=3.5cm},
        cat3/.style={box, fill=catVerdeMenta!40, text width=3cm},
...
Para abordar rigurosamente la asignación de crédito y la no estacionariedad sin sacrificar la escalabilidad de la ejecución descentralizada, los métodos Actor-Crítico bajo el paradigma CTDE se posicionan como esquemas altamente adoptados. A diferencia de métodos que descomponen el valor conjunto en espacios de acción discretos (como VDN o QMIX \cite{sunehag2017value,rashid2018qmix}), estas aproximaciones parametrizan y optimizan directamente la política.

\begin{itemize}
    \item \textbf{\textit{Multi-Agent PPO} (MAPPO)}: Es la adaptación multi-agente de PPO estructurada sobre el Teorema del Gradiente de Política Multi-Agente \cite{yu2022surprising}. MAPPO desacopla el aprendizaje utilizando un Crítico Centralizado global (empleado solo en entrenamiento) y Actores neuronales descentralizados. Al procesar el estado global (por ejemplo, la distribución total del tránsito en toda la red), el Crítico actúa como un oráculo que elimina la incertidumbre introducida por la observabilidad parcial. De esta forma, computa una Función de Ventaja analíticamente certera que resuelve de manera explícita el problema de la asignación de crédito multi-agente, evaluando el efecto marginal real de cada decisión local sobre el objetivo de fluidez global.
    \item \textbf{\textit{Heterogeneous-Agent PPO} (HAPPO)}: Incorpora una innovación algorítmica fundamental para redes urbanas con topologías físicas asimétricas (por ejemplo, intersecciones céntricas altamente congestionadas frente a vías de tránsito rápido perimetrales) \cite{kuba2021trust}. En aproximaciones estándar de CTDE, la optimización de recursos computacionales frecuentemente impone la Compartición de Parámetros (\textit{Parameter Sharing}), forzando a los agentes a utilizar pesos neuronales idénticos que homogeneizan su comportamiento. En escenarios asimétricos, esta homogenización impide la especialización táctica requerida por cada intersección. HAPPO supera esta limitación descartando la compartición de parámetros a favor de una Compartición de Experiencia (\textit{Experience Sharing}) fundamentada en la Descomposición de Ventaja Multi-Agente. Esta formulación permite desplegar arquitecturas de agentes estrictamente heterogéneas transfiriendo trayectorias temporales robustas entre ellos, y penalizando los desajustes analíticos originados por evaluar políticas ajenas (\textit{off-policy}) mediante factores correctivos de Muestreo de Importancia (\textit{Importance Sampling}). Este mecanismo garantiza un progreso monótono en la optimización conjunta, preservando la pluralidad funcional indispensable en una red semaforizada heterogénea.
\end{itemize}
\newpage
