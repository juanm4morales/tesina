...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption[Movimientos vehiculares, fases y cruces peatonales]{Convención esquemática de movimientos vehiculares, fases y cruces peatonales concurrentes en una intersección de cuatro accesos, adaptada conceptualmente del manual de temporización semafórica de FHWA \cite{fhwa_signal_timing_manual}.}
...
La simulación de tránsito constituye una representación abstracta del fenómeno real. Su objetivo no es reproducir visualmente una escena particular, sino modelar, mediante reglas de comportamiento y variables observables, los procesos que determinan la evolución de la circulación. La calidad de sus resultados depende de la representación de la red, la demanda, la flota, los modelos de comportamiento y las reglas de control utilizadas. Por ello, la simulación debe entenderse como un entorno experimental sujeto a supuestos y parametrización, cuyos resultados requieren una evaluación operacional acorde con el objetivo del estudio.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{imagenes/SIMvsREAL.png}
    \caption{Correspondencia conceptual entre una situación real de circulación vehicular y su representación microscópica en un simulador. Elaboración propia.}
    \label{fig:sim_vs_real}
\end{figure}

Para el presente trabajo, el nivel de mayor interés es el microscópico, ya que representa explícitamente a cada vehículo, su ruta y su movimiento dentro de la red \cite{fernandez}. \gls{acr-sumo} se ubica en esta categoría. Es una suite libre y de código abierto iniciada en el Instituto de Sistemas de Transporte del \gls{acr-dlr}, que permite modelar sistemas de tránsito intermodales, incluidos vehículos, transporte público y peatones. También integra herramientas para importar redes, calcular rutas, visualizar y evaluar simulaciones, además de interfaces para controlar remotamente su ejecución \cite{SUMO2018_SUMO,sumo-doc}. Estas características han permitido emplearlo, entre otras aplicaciones, en la evaluación de semáforos y en el entrenamiento de planes de control. En SUMO, la evolución vehicular se actualiza en pasos temporales discretos sobre una representación espacial continua; además, el entorno permite modelar calles con múltiples carriles, cambios de carril, distintos tipos de vehículos, rutas individuales, semáforos y salidas de simulación a nivel de vehículo, carril, arco o detector \cite{SUMO2018_SUMO,sumo-doc}.

La interfaz gráfica \textit{sumo-gui} permite inspeccionar visualmente la red, los vehículos y algunos valores de la simulación durante la ejecución. Esta visualización cumple una función de apoyo para la inspección del escenario y la depuración del modelo; las métricas utilizadas para la evaluación se obtienen de las salidas de simulación y de las interfaces de control \cite{sumo-doc}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{imagenes/SUMO_GUI_background.png}
    \caption{Interfaz gráfica de SUMO durante la ejecución de una simulación microscópica, con visualización de la red y de variables del vehículo.}
    \label{fig:sumo_gui}
\end{figure}
...
\subsubsection{Representación Microscópica de Intersecciones en SUMO}
...
En este contexto, el simulador de tránsito constituye el entorno operativo sobre el cual se formaliza el problema de control semafórico. Para el esquema de la Figura \ref{fig:sumo_intersection_elements}, que adopta circulación por la derecha, se distinguen carriles, líneas de detención, conexiones entre carriles y estados de control semafórico \cite{sumo-doc}:
\begin{itemize}
...
La suite SUMO reúne aplicaciones para construir redes, generar demandas, ejecutar simulaciones, visualizar sus resultados y controlar la ejecución mediante interfaces externas \cite{SUMO2018_SUMO,sumo-doc}:
\begin{itemize}
    \item \textbf{\textit{netconvert}:} Aplicación que importa redes desde distintas fuentes y compila su representación en el formato nativo de SUMO. También permite reconstruir o ajustar elementos de la red, como las conexiones y los programas semafóricos.
    ...
    \item \textbf{\textit{osmWebWizard}:} Herramienta que automatiza la generación de un escenario a partir de un área seleccionada de OpenStreetMap. Se considera una alternativa de importación y no reemplaza el procesamiento específico realizado con \textit{netconvert}.
    ...
    \item \textbf{\textit{libsumo}:} Implementación embebida que ofrece la interfaz de programación de SUMO sin la comunicación mediante sockets de la modalidad TraCI estándar. Esta alternativa resulta útil para integrar el simulador con controladores externos y reducir la sobrecarga de comunicación.
\end{itemize}

Desde la perspectiva del modelado computacional, una simulación SUMO se construye a partir de archivos \gls{acr-xml} de red, demanda, elementos adicionales y configuración \cite{sumo-doc}. Los archivos \gls{acr-taz} pueden utilizarse durante la estructuración espacial y la generación de demanda, mientras que las salidas de la simulación se solicitan mediante opciones de configuración específicas. Sus funciones generales se resumen en la Tabla \ref{tab:sumo_input_files}:

\begin{table}[H]
\centering
\caption{Archivos de entrada y configuración empleados en una simulación SUMO.}
...
Zonas TAZ & \texttt{.taz.xml} & Zonas para estructurar espacialmente la demanda. \\ \hline
...
\begin{itemize}
    ...
    \item \textbf{Emisiones de $\text{CO}_2$:} SUMO estima las emisiones a partir del estado cinemático de los vehículos, su tipo y la clase de emisiones configurada \cite{sumo-doc}. La masa total debe interpretarse junto con los vehículos completados y la distancia recorrida, o expresarse mediante una medida normalizada.
    \item \textbf{Indicadores de validez de la simulación:} Los teletransportes, los vehículos pendientes y otras incidencias de ejecución permiten detectar bloqueos o condiciones que comprometen la comparabilidad de una corrida.
\end{itemize}
...
El aprendizaje automático (\textit{Machine Learning}, ML) suele dividirse en tres paradigmas. En el aprendizaje supervisado (\textit{Supervised Learning}, SL), el modelo se entrena con ejemplos etiquetados que indican la salida esperada. En el aprendizaje no supervisado (\textit{Unsupervised Learning}, UL), se busca descubrir estructura en datos no etiquetados, por ejemplo mediante agrupamiento o reducción de dimensionalidad. El aprendizaje por refuerzo (\textit{Reinforcement Learning}, RL) aborda un problema distinto: una entidad aprende mediante interacción con un entorno qué acciones elegir para maximizar una recompensa acumulada \cite{suttonRL}.
...
En una formulación actor-crítico de un paso, después de ejecutar $A_t$ en $S_t$, el entorno devuelve la recompensa $R_{t+1}$, el estado $S_{t+1}$ y el indicador terminal $d_{t+1}$. El crítico calcula el error de diferencia temporal
\begin{equation}
    \delta_t = R_{t+1} + \gamma(1-d_{t+1})V_\phi(S_{t+1}) - V_\phi(S_t),
    \label{eq:actor_critic_td}
\end{equation}
donde el factor $(1-d_{t+1})$ elimina el valor del estado siguiente cuando finaliza el episodio. Si $V_\phi$ aproxima correctamente a $V^\pi$, $\delta_t$ constituye una estimación de un paso de la ventaja de la acción ejecutada. Por ello, un valor positivo indica un resultado superior al esperado para ese estado y uno negativo indica lo contrario. El crítico ajusta $\phi$ para reducir el error entre $V_\phi(S_t)$ y el objetivo temporal $R_{t+1}+\gamma(1-d_{t+1})V_\phi(S_{t+1})$, mientras que el actor utiliza la misma señal para reforzar o desalentar la acción observada:
\begin{equation}
    \theta \leftarrow \theta + \alpha_\theta\,\delta_t\,
    \nabla_\theta \log \pi_\theta(A_t\mid S_t).
    \label{eq:actor_critic_update}
\end{equation}
Esta forma canónica permite explicar el intercambio de información entre ambos componentes, aunque los algoritmos modernos suelen sustituir $\delta_t$ por estimadores de ventaja que combinan varios pasos, como \gls{acr-gae}, y realizan las actualizaciones sobre mini-lotes.

La Figura \ref{fig:actor_critic} resume este circuito para el control semafórico. La observación del tránsito alimenta al actor y al crítico; el actor selecciona una fase, el entorno simulado produce la siguiente observación y la recompensa, y el crítico transforma esa transición en una señal de evaluación. Esta señal guía el aprendizaje, pero no se utiliza como una orden directa sobre el entorno.

\begin{figure}[H]
    \centering
    \resizebox{0.96\textwidth}{!}{%
    \begin{tikzpicture}[
        bloque/.style={draw, rounded corners=4pt, thick, align=center, minimum height=1.0cm, font=\small},
        dato/.style={bloque, fill=colDatoPrimario!16, draw=colDatoPrimario!80!black, text width=2.5cm},
        actor/.style={bloque, fill=colAgente!22, draw=colAgente!80!black, text width=3.0cm},
        critico/.style={bloque, fill=catMelocoton!45, draw=catMelocoton!80!black, text width=3.0cm},
        entorno/.style={bloque, fill=colEntorno!22, draw=colEntorno!80!black, text width=3.0cm},
        senal/.style={bloque, fill=colInterfaz!25, draw=colInterfaz!80!black, text width=3.2cm},
        flecha/.style={-{Stealth[length=5pt]}, thick, draw=black!80},
        retorno/.style={flecha, dashed, draw=black!65},
        etiqueta/.style={font=\scriptsize, fill=white, inner sep=1.5pt, align=center}
    ]
        \node[dato] (obs) at (-5.7,0) {Observación de tránsito\\$S_t$};
        \node[actor] (actor) at (-1.7,1.55) {\textbf{Actor}\\política $\pi_\theta(a\mid S_t)$};
        \node[critico] (critic) at (-1.7,-1.55) {\textbf{Crítico}\\valor $V_\phi(S_t)$};
        \node[entorno] (env) at (4.1,1.55) {\textbf{Entorno}\\red vial en SUMO};
        \node[senal] (td) at (4.1,-1.55) {Error \gls{acr-td} / ventaja\\$\delta_t$ o $\hat A_t$};

        \draw[flecha] (obs) |- (actor.west);
        \draw[flecha] (obs) |- (critic.west);
        \draw[flecha] (actor) -- node[etiqueta, above] {acción: fase $A_t$} (env);
        \draw[flecha] (env.south) -- node[etiqueta, right=2pt] {$R_{t+1},S_{t+1},d_{t+1}$} (td.north);
        \draw[flecha] (critic) -- node[etiqueta, above] {$V_\phi(S_t)$} (td);
        \draw[retorno] (td.north west) to[out=145,in=-35]
            node[etiqueta, pos=0.52, above left=1pt] {actualiza $\theta$} (actor.south east);
        \draw[retorno] (td.south) -- ++(0,-0.65) -|
            node[etiqueta, pos=0.75, below] {actualiza $\phi$} (critic.south);
        \draw[retorno] (env.north) -- ++(0,0.85) -|
            node[etiqueta, pos=0.25, above] {nueva observación} (obs.north);
    \end{tikzpicture}%
    }
...
La Figura \ref{fig:rl_taxonomy} ubica los algoritmos evaluados dentro de las familias de métodos libres de modelo basados en valor y basados en política.
...
En problemas de control complejos, los métodos tabulares se vuelven impracticables debido a las restricciones de memoria y cómputo asociadas con espacios de estados grandes o continuos \cite{suttonRL}. La aproximación de funciones aborda esta limitación mediante una representación parametrizada, de modo que la información aprendida en una situación pueda generalizarse a otras situaciones con características similares. Las redes neuronales constituyen una familia de aproximadores de funciones que puede utilizarse para este propósito \cite{bishop2024deep}.

El componente básico de una red neuronal es la unidad neuronal individual. Dada una entrada $\mathbf{x}$, la unidad asigna un peso a cada componente de esa entrada, suma un sesgo y transforma el resultado mediante una función de activación. La suma ponderada más el sesgo se denomina preactivación, mientras que la salida de la función de activación constituye la respuesta de la unidad \cite{goodfellow2016deep}.
...
En la ecuación \ref{eq:neurona_artificial}, $\mathbf{x}_{k-1}$ es el vector de entrada a la capa $k$, $\mathbf{w}_{k,u}$ contiene los pesos asociados con la unidad $u$, $b_{k,u}$ es el sesgo y $g_k$ es la función de activación. La función de activación introduce la no linealidad necesaria: la composición de transformaciones exclusivamente lineales podría reducirse a una única transformación lineal, aun cuando se utilizaran varias capas \cite{goodfellow2016deep}.

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        >=Latex,
        node distance=1.35cm and 1.4cm,
        unit/.style={draw=colDatoPrimario!80!black, fill=colDatoPrimario!15, rounded corners, minimum width=1.5cm, minimum height=0.75cm, align=center},
        operation/.style={draw=colAgente!80!black, fill=colAgente!15, rounded corners, minimum width=2.1cm, minimum height=0.85cm, align=center},
        output/.style={draw=colInterfaz!80!black, fill=colInterfaz!25, rounded corners, minimum width=1.8cm, minimum height=0.75cm, align=center}
    ]
        \node[unit] (x1) {$x_1$};
        \node[unit, below=0.25cm of x1] (x2) {$x_2$};
        \node[unit, below=0.25cm of x2] (xd) {$\vdots$};
        \node[unit, below=0.25cm of xd] (xdn) {$x_d$};
        \node[operation, right=of xd] (affine) {$a=\mathbf{w}^{\top}\mathbf{x}+b$};
        \node[operation, right=of affine] (activation) {$h=g(a)$};
        \node[output, right=of activation] (out) {salida};

        \draw[->] (x1.east) -- (affine.west);
        \draw[->] (x2.east) -- (affine.west);
        \draw[->] (xd.east) -- (affine.west);
        \draw[->] (xdn.east) -- (affine.west);
        \draw[->] (affine) -- (activation);
        \draw[->] (activation) -- (out);
        \node[above=0.12cm of affine, font=\small] {pesos y sesgo};
    \end{tikzpicture}
    \caption{Operaciones de una unidad neuronal artificial: combinación afín de las entradas y aplicación de una función de activación.}
    \label{fig:neurona_artificial}
\end{figure}

Las funciones de activación más habituales en las capas ocultas consideradas aquí son \cite{goodfellow2016deep}:
\begin{itemize}
    \item \textbf{ReLU:} $g(z)=\max\{0,z\}$. Es sencilla de optimizar y conserva una derivada no nula en la región activa, aunque no es diferenciable exactamente en $z=0$; en la práctica se utiliza una derivada lateral o subgradiente.
    \item \textbf{\textit{Leaky} ReLU:} $g(z)=\max\{0,z\}+\alpha\min\{0,z\}$, donde $\alpha$ es un valor pequeño y positivo. La pendiente en la región negativa permite que la unidad reciba gradiente aun cuando su activación sea negativa.
    \item \textbf{Tanh:} $g(z)=\tanh(z)$, con valores de salida en $[-1,1]$. Es simétrica respecto del origen, pero puede saturarse para valores extremos y dificultar el aprendizaje basado en gradientes.
\end{itemize}

Las redes \textit{feedforward} organizan sus unidades en capas acíclicas. Cada capa recibe la salida de la anterior, aplica una transformación afín y, salvo cuando la tarea requiere una salida lineal, una función de activación. Para una red con $L$ capas parametrizadas, esta operación puede escribirse como:
\begin{equation}
    \mathbf{x}_k = g_k\!\left(\mathbf{W}_k^{\top}\mathbf{x}_{k-1}+\mathbf{b}_k\right),
    \qquad k=1,\ldots,L,
    \qquad \mathbf{x}_0=\mathbf{x}
    \label{eq:red_feedforward}
\end{equation}
Aquí, $\mathbf{W}_k$ y $\mathbf{b}_k$ son los parámetros de la capa $k$, $\mathbf{x}_{k-1}$ es la salida de la capa anterior y $g_k$ se aplica componente a componente. La arquitectura suele describirse mediante una capa de entrada, una o más capas ocultas y una capa de salida, aunque el número de capas se cuenta de acuerdo con la convención adoptada para los parámetros entrenables \cite{bishop2024deep,goodfellow2016deep}. La ecuación \ref{eq:red_feedforward} resume la composición de funciones que permite construir una red profunda.

La Figura \ref{fig:red_feedforward} representa esta organización para una red con dos capas ocultas. Las flechas avanzan únicamente hacia la derecha: cada unidad de una capa recibe las salidas de la capa anterior y las transforma mediante los parámetros de esa capa, sin conexiones recurrentes.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        >=Stealth,
        neurona/.style={circle, draw=colDatoPrimario!80!black, fill=colDatoPrimario!15, thick, minimum size=0.62cm, inner sep=0pt},
        oculta/.style={circle, draw=colAgente!80!black, fill=colAgente!15, thick, minimum size=0.62cm, inner sep=0pt},
        salida/.style={circle, draw=colInterfaz!80!black, fill=colInterfaz!25, thick, minimum size=0.62cm, inner sep=0pt},
        conexion/.style={-{Stealth[length=4pt]}, draw=black!65, thin},
        titulo/.style={font=\small\bfseries},
        anotacion/.style={font=\scriptsize, align=center}
    ]
        % Layout: four aligned columns with left-to-right acyclic flow.
        \node[titulo] at (0,2.35) {Entrada};
        \node[titulo] at (3.1,2.35) {Capa oculta 1};
        \node[titulo] at (6.2,2.35) {Capa oculta 2};
        \node[titulo] at (9.3,2.35) {Salida};

        \node[neurona] (i1) at (0,1.25) {$x_1$};
        \node[neurona] (i2) at (0,0) {$x_2$};
        \node[neurona] (id) at (0,-1.25) {$\vdots$};

        \node[oculta] (h11) at (3.1,1.8) {$x_{1,1}$};
        \node[oculta] (h12) at (3.1,0.6) {$x_{1,2}$};
        \node[oculta] (h13) at (3.1,-0.6) {$\vdots$};
        \node[oculta] (h14) at (3.1,-1.8) {$x_{1,m}$};

        \node[oculta] (h21) at (6.2,1.8) {$x_{2,1}$};
        \node[oculta] (h22) at (6.2,0.6) {$x_{2,2}$};
        \node[oculta] (h23) at (6.2,-0.6) {$\vdots$};
        \node[oculta] (h24) at (6.2,-1.8) {$x_{2,q}$};

        \node[salida] (o1) at (9.3,0.65) {$x_{L,1}$};
        \node[salida] (o2) at (9.3,-0.65) {$x_{L,r}$};

        \foreach \source in {i1,i2,id}
            \foreach \target in {h11,h12,h13,h14}
                \draw[conexion] (\source) -- (\target);
        \foreach \source in {h11,h12,h13,h14}
            \foreach \target in {h21,h22,h23,h24}
                \draw[conexion] (\source) -- (\target);
        \foreach \source in {h21,h22,h23,h24}
            \foreach \target in {o1,o2}
                \draw[conexion] (\source) -- (\target);

        \node[anotacion] at (3.1,-2.55) {$\mathbf{x}_1=g_1(\cdot)$};
        \node[anotacion] at (6.2,-2.55) {$\mathbf{x}_2=g_2(\cdot)$};
        \node[anotacion] at (9.3,-2.55) {$\mathbf{x}_L=g_L(\cdot)$};
    \end{tikzpicture}%
    }
    \caption{Arquitectura conceptual de una red neuronal \textit{feedforward}. Cada columna representa una capa y las conexiones se dirigen desde la entrada hacia la salida; las expresiones inferiores indican la transformación afín seguida de la activación en cada capa.}
    \label{fig:red_feedforward}
\end{figure}

El Teorema de Aproximación Universal es un resultado de existencia: bajo condiciones apropiadas, una red de dos capas parametrizadas, con una capa oculta suficientemente ancha, puede aproximar una función continua sobre un dominio compacto con una precisión arbitraria. El teorema no proporciona por sí mismo un procedimiento para encontrar los parámetros, no garantiza un entrenamiento eficiente y tampoco implica una buena generalización sobre datos no observados. La profundidad puede ofrecer representaciones jerárquicas y una mayor eficiencia representacional para ciertas familias de funciones, pero no garantiza por sí sola un menor error de prueba \cite{bishop2024deep}.

\subsubsection{El Proceso de Optimización}

Dado un conjunto de datos y una función de pérdida diferenciable $\mathcal{L}(\theta)$, los parámetros $\theta$ se ajustan mediante un procedimiento de optimización iterativa. En términos generales, el descenso de gradiente actualiza los parámetros según:
\begin{equation}
    \theta_{t+1}=\theta_t-\eta\nabla_{\theta}\mathcal{L}(\theta_t),
    \label{eq:descenso_gradiente}
\end{equation}
donde $\eta$ es la tasa de aprendizaje. En la práctica, el gradiente puede calcularse sobre todo el conjunto de entrenamiento, sobre una observación individual o sobre un mini-lote; el tamaño del mini-lote modifica el costo computacional y la variabilidad de la estimación, pero no constituye un equilibrio óptimo garantizado \cite{bishop2024deep}.

\begin{itemize}
    \item \textbf{Función de pérdida (\textit{loss function}):} cuantifica la discrepancia entre las predicciones de la red y los valores objetivo. Puede adoptar formas distintas según la tarea; por ejemplo, el error cuadrático medio en regresión o la entropía cruzada en clasificación. En DQN, la pérdida se formula a partir del error de diferencia temporal presentado más adelante.
    \item \textbf{Descenso por mini-lotes:} utiliza un subconjunto de datos para estimar el gradiente en cada actualización. El barajado de las observaciones puede reducir correlaciones debidas al orden de los datos, pero el procedimiento no garantiza por sí mismo estabilidad ni convergencia.
...
La Figura \ref{fig:arquitectura_dqn} distingue el circuito de interacción del circuito de entrenamiento. Durante la interacción, la red en línea transforma el estado del tránsito en un valor para cada fase disponible y una regla $\epsilon$-greedy selecciona la acción. La transición resultante se almacena en el búfer. Durante el entrenamiento, un mini-lote alimenta tanto la predicción de la red en línea como el cálculo del objetivo mediante la red objetivo; solo los parámetros $\theta$ se modifican en cada paso, mientras que $\theta^-$ se reemplaza periódicamente por una copia de $\theta$.

\begin{figure}[H]
    \centering
    \resizebox{0.98\textwidth}{!}{%
    \begin{tikzpicture}[
        bloque/.style={draw, rounded corners=4pt, thick, align=center, minimum height=0.9cm, font=\small},
        ...
        \node[salida] (select) at (3.1,2.2) {Selección\\$\epsilon$-greedy};
        \node[entorno] (env) at (6.4,2.2) {SUMO\\ejecuta $A_t$};
        ...
        \draw[flecha] (buffer) -- node[etiqueta, above] {mini-lote} (target);
    ...
    \caption{Flujo conceptual de DQN con repetición de experiencias y red objetivo. La red en línea selecciona acciones y recibe las actualizaciones; la red objetivo proporciona el término de \textit{bootstrapping} del blanco \gls{acr-td}. Elaboración propia a partir del algoritmo descrito por Mnih et al. \cite{mnih2015}.}
    \label{fig:arquitectura_dqn}
\end{figure}

La función de pérdida minimizada por DQN, representada en el bloque inferior de la Figura \ref{fig:arquitectura_dqn}, es:
\begin{equation}
...
\end{equation}
donde $\mathcal{D}$ es el búfer de repetición de experiencias, $(s_j, a_j, r_{j+1}, s_{j+1}, d_{j+1})$ representa una transición muestreada, $d_{j+1}$ vale uno cuando la transición es terminal y cero en caso contrario, $\theta$ indica los parámetros de la red $Q$ actual y $\theta^-$ representa los parámetros congelados de la red objetivo.

El procedimiento conceptual de DQN combina la recolección de transiciones, la selección de acciones mediante una política $\epsilon$-greedy y la actualización de la red a partir de mini-lotes extraídos del búfer de experiencias. El Algoritmo \ref{alg:dqn} resume este ciclo sin imponer decisiones particulares de implementación:
...
Como alternativa a derivar la política de forma indirecta a partir de una función de valor (como en DQN), DRL permite parametrizar directamente la política del agente mediante una red neuronal $\pi(a|s; \theta)$ \cite{marlbook}. En un problema de acciones discretas, la red recibe el estado u observación, calcula un puntaje para cada acción y transforma esos puntajes en una distribución de probabilidad, habitualmente mediante una función \textit{softmax}. La acción puede muestrearse de esa distribución durante el entrenamiento, preservando la exploración estocástica.

La Figura \ref{fig:red_politica} muestra este recorrido para un controlador semafórico. A diferencia de DQN, las salidas no son estimaciones de valor: son probabilidades que suman uno. La señal de ventaja no entra a la red para seleccionar la fase; se utiliza durante el entrenamiento para modificar los parámetros en la dirección indicada por el gradiente de la log-probabilidad de la acción observada.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        bloque/.style={draw, rounded corners=4pt, thick, align=center, minimum height=0.95cm, font=\small},
        ...
        \node[ventaja, below=1.3cm of probs] (adv) {Señal de aprendizaje\\ventaja estimada $\hat A_t$};
...
\begin{equation}
    ...
    \label{eq:gae}
\end{equation}
donde $\delta_t^V = R_{t+1} + \gamma(1-d_{t+1})V(S_{t+1}) - V(S_t)$ representa el error de \gls{acr-td} estimado por la red del crítico $V$, $d_{t+1}$ indica si la transición es terminal, $\gamma \in [0, 1)$ es el factor de descuento y $\lambda \in [0, 1]$ es el hiperparámetro de suavizado \gls{acr-gae} que equilibra el sesgo y la varianza del estimador.

En lugar de minimizar un error cuadrático de predicción, la optimización paramétrica del actor se rige por el Teorema del Gradiente de Política (\textit{Policy Gradient Theorem}), que establece la actualización de los parámetros $\theta$ ascendiendo por el gradiente del rendimiento esperado \cite{suttonRL}. El operador $\hat{\mathbb{E}}_t$ denota el promedio empírico sobre las transiciones del lote recolectado en los instantes $t$:
\begin{equation}
    \nabla_\theta J(\theta) = \hat{\mathbb{E}}_t \left[ \nabla_\theta \log \pi_\theta(A_t \mid S_t) \hat{A}_t \right]
    \label{eq:gradiente_politica}
\end{equation}
donde $\nabla_\theta J(\theta)$ representa el gradiente del desempeño esperado, $\pi_\theta(A_t \mid S_t)$ es la probabilidad otorgada a la acción ejecutada $A_t$ en el estado $S_t$, y $\hat{A}_t$ es la ventaja estimada. Como ilustra la Figura \ref{fig:red_politica}, el término $\nabla_\theta\log\pi_\theta(A_t\mid S_t)$ indica cómo modificar los parámetros para cambiar la probabilidad de la acción observada, mientras que $\hat A_t$ determina el signo y la magnitud de esa modificación. Por lo tanto, la Ecuación \ref{eq:gradiente_politica} incrementa la probabilidad de elegir acciones con ventaja positiva y reduce la de aquellas con ventaja negativa.
...
\end{equation}
donde $\phi$ representa los parámetros de la red del crítico $V_\phi$, $V_t^{\text{target}}$ es el retorno objetivo estimado para ajustar el crítico, $S[\pi_\theta]$ denota la entropía de la distribución de la política, $c_1$ pondera el error de valor y $c_2$ pondera el incentivo a conservar exploración, con $c_1, c_2 > 0$.

PPO es un algoritmo predominantemente \textit{on-policy} que reutiliza de forma limitada el lote recolectado con la política vigente. En esta investigación constituye la base conceptual de IPPO (\textit{Independent Proximal Policy Optimization}), MAPPO (\textit{Multi-Agent Proximal Policy Optimization}) y HAPPO (\textit{Heterogeneous-Agent Proximal Policy Optimization}); las diferencias entre estas variantes se explican en la sección multiagente \cite{yu2022surprising,kuba2021trust}.

El Algoritmo \ref{alg:ppo} resume el ciclo conceptual de PPO. La recolección de trayectorias se realiza con la política vigente y el lote se reutiliza durante un número limitado de épocas; posteriormente se descarta para obtener nuevas muestras con la política actualizada:
...
La formalización matemática de los entornos multi-agente requiere extender el \gls{acr-mdp} individual incorporando nociones de la Teoría de Juegos. Liang et al. \cite{liang2025marlreview} destacan que, cuando las acciones de los agentes modifican conjuntamente los estados y recompensas, el modelo individual deja de describir adecuadamente la interacción; por ello, el modelo matemático fundamental para representar decisiones secuenciales con múltiples participantes es el Juego Estocástico o Juego Markoviano (\textit{Stochastic Game} o \textit{Markov Game}), introducido por Shapley \cite{shapley1953stochastic} y adaptado al aprendizaje por refuerzo por Albrecht et al. \cite{marlbook}.
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption[Interacción agente-entorno en MARL cooperativo]{Interacción Agente-Entorno en un esquema MARL cooperativo. Cada semáforo emite una acción local ($a_i$) formando una acción conjunta ($\mathbf{a}$) que modifica el estado global del entorno de tráfico.}
    \label{fig:marl_interaction}
\end{figure}

La Figura \ref{fig:marl_interaction} muestra cómo las acciones locales de los controladores se combinan antes de modificar el estado global de la red.

\subsection{Desafíos Fundamentales en MARL}

La aplicación práctica de métodos de aprendizaje por refuerzo en escenarios multi-agente enfrenta obstáculos analíticos y computacionales significativos que no existen en el caso individual \cite{marlbook}. A medida que aumenta el número de agentes, el espacio de acciones conjunto $\boldsymbol{\mathcal{A}}$ crece de forma exponencial con el número de agentes ($|\boldsymbol{\mathcal{A}}| = \prod_{i=1}^n |\mathcal{A}_i|$), lo que introduce un crecimiento combinatorio del espacio de decisiones. Sin embargo, los desafíos teóricos más severos radican en la dinámica propia de la interacción:
...
\begin{itemize}
    ...
    Este fenómeno fue formalizado por Bowling y Veloso \cite{bowling2002multiagent} como el problema del \textit{blanco móvil} (\textit{moving target}): una política que es óptima frente a los demás participantes en un momento dado deja de serlo cuando estos se adaptan. En juegos matriciales como Piedra, Papel o Tijera, el aprendizaje independiente con tasas de actualización fijas provoca oscilaciones continuas en torno al equilibrio sin converger. Como ilustra la Figura \ref{fig:no_estacionariedad_papel_piedra}, al aplicar algoritmos como WoLF-PHC (\textit{Win or Learn Fast Policy Hill-Climbing}), las trayectorias de ambos agentes amortiguan sus oscilaciones y espiralan hacia la política convergida en el equilibrio de Nash ($\bigstar = (1/3, 1/3, 1/3)$).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{imagenes/non_stationary_papel_piedra.jpeg}
    \caption[Evolución de las políticas de dos agentes bajo WoLF-PHC en Piedra, Papel o Tijera]{Evolución de las políticas de dos agentes con el algoritmo WoLF-PHC en el juego de Piedra, Papel o Tijera. La diagonal discontinua divide el gráfico en dos símplex de probabilidad, uno para cada agente. Cada punto en el símplex representa una distribución sobre las tres acciones disponibles; las trayectorias muestran el avance de las políticas durante el entrenamiento y las estrellas ($\bigstar$) señalan las políticas convergidas en el equilibrio de Nash. Adaptado de \cite{bowling2002multiagent,marlbook}.}
...
Para abordar estos desafíos en escenarios \gls{acr-decpomdp}, la literatura clasifica los algoritmos en función de cómo fluye la información durante el proceso de optimización. Se distinguen fundamentalmente dos enfoques: el Aprendizaje Independiente (\textit{Independent Learning}, IL) y el \gls{acr-ctde} \cite{ctde_survey}.
...
\subsection{MADRL y líneas de base}
...
En esta investigación, para evaluar rigurosamente el desempeño de las arquitecturas de entrenamiento centralizado (MAPPO y HAPPO), resulta metodológicamente conveniente establecer \textbf{líneas de base de aprendizaje independiente (IL)}. En los algoritmos independientes, cada controlador actúa como un agente aislado que no modela la presencia explícita de los demás participantes.

\subsubsection{Aprendizaje Independiente (IL): IDQN e IPPO como Líneas de Base}
...
\subsection{HAPPO}\label{subsec:happo}
...
La Figura \ref{fig:happo_sequential} ilustra el orden secuencial de actualización que distingue a HAPPO de una actualización independiente simultánea. En ella, $M$ representa una señal de ventaja reponderada por los cambios de los agentes precedentes, no un cociente de probabilidad aislado.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \draw[arr, colDatoPrimario, thick] (pi1new.east) -- node[above=2pt, align=center, font=\scriptsize, text=colDatoPrimario!90!black] {Factor de ventaja\\$M_{i_2}$} (pi2new.west);
        \draw[arr, colDatoPrimario, thick] (pi2new.east) -- node[above=2pt, align=center, font=\scriptsize, text=colDatoPrimario!90!black] {Factor de ventaja\\$M_{i_n}$} (pinnew.west);
        
        \node[font=\Large, text=black!50] at (7.8, 1.25) {$\dots$};
    \end{tikzpicture}
    \caption{Esquema de actualización secuencial de HAPPO. A diferencia de PPO independiente, la actualización de cada agente está condicionada por el factor de ventaja reponderada $M_{i_m}$, que incorpora los cambios efectuados por los agentes que lo precedieron en la secuencia.}
