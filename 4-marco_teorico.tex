...
Para la generación, edición y control en tiempo real de los escenarios de simulación microscópica, el entorno SUMO se complementa con un conjunto integrado de herramientas y librerías auxiliares \cite{SUMO2018_SUMO}:
\begin{itemize}
...
Una suposición habitual del MDP es que el agente observa un estado suficientemente informativo del entorno. Sin embargo, en muchas aplicaciones reales el agente solo recibe observaciones parciales o ruidosas. En estos casos, el marco se generaliza a un Proceso de Decisión de Markov Parcialmente Observable (\textit{Partially Observable Markov Decision Process}, POMDP), donde el historial de observaciones puede ser necesario para inferir el estado subyacente \cite{marlbook}.
...
\end{equation}
Esta forma canónica permite explicar el intercambio de información entre ambos componentes, aunque los algoritmos modernos suelen sustituir $\delta_t$ por estimadores de ventaja que combinan varios pasos, como GAE, y realizan las actualizaciones sobre mini-lotes.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[senal] (td) at (4.1,-1.55) {Error TD / ventaja\\$\delta_t$ o $\hat A_t$};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[salida] (loss) at (-4.15,-1.55) {Pérdida TD\\$(y-Q_t)^2$};
    ...
    \caption{Flujo conceptual de DQN con repetición de experiencias y red objetivo. La red en línea selecciona acciones y recibe las actualizaciones; la red objetivo proporciona el término de \textit{bootstrapping} del blanco TD. Elaboración propia a partir del algoritmo descrito por Mnih et al. \cite{mnih2015}.}
...
&\underbrace{r_{j+1} + \gamma(1-d_{j+1}) \max_{a'} \hat{Q}(s_{j+1}, a'; \theta^{-})}_{\text{Valor objetivo de TD}} \\
...
\begin{equation}
    ...
    \label{eq:gae}
\end{equation}
donde $\delta_t^V = R_{t+1} + \gamma(1-d_{t+1})V(S_{t+1}) - V(S_t)$ representa el error de diferencia temporal (TD) estimado por la red del crítico $V$, $d_{t+1}$ indica si la transición es terminal, $\gamma \in [0, 1)$ es el factor de descuento y $\lambda \in [0, 1]$ es el hiperparámetro de suavizado GAE que equilibra el sesgo y la varianza del estimador.
...
En escenarios reales de control distribuido, ningún agente dispone de visibilidad completa del estado global del sistema. Cada participante recopila información únicamente de sus sensores locales. Por consiguiente, los entornos cooperativos multi-agente con observabilidad parcial se formalizan matemáticamente mediante el marco de los Procesos de Decisión de Markov Parcialmente Observables Descentralizados (Dec-POMDP) \cite{decpomdp-survey}.
