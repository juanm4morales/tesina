...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        box/.style={rectangle, rounded corners=4pt, thick, align=center, minimum width=2.7cm, minimum height=1.0cm, font=\small},
        ...
        \node[env] (sumo) {Entorno microscópico\\SUMO / red vial};
        \node[obs, right=of sumo] (observ) {Observaciones locales\\$o_i$: colas, velocidad, fase};
        \node[ag, right=of observ] (actor) {Actor local\\$\pi_i(a_i\mid o_i)$};
        \node[env, right=of actor] (fase) {Acción semafórica\\$a_i$: fase válida};
        \node[rew, below=1.0cm of actor] (reward) {Recompensa\\demora, cola, detenciones};
        \node[rew, below=1.0cm of sumo] (critic) {Durante CTDE\\crítico con información global};

        \draw[arr] (sumo) -- node[lbl] {telemetría} (observ);
        \draw[arr] (observ) -- (actor);
        \draw[arr] (actor) -- (fase);
        \draw[arr] (fase) |- (sumo);
        \draw[arr] (sumo) |- (reward);
        \draw[arr] (reward) -| node[lbl, below] {actualización} (actor);
        \draw[arr, dashed] (sumo) -- node[lbl, left] {estado global} (critic);
        \draw[arr, dashed] (critic) -| node[lbl, right] {valor / ventaja} (actor);
    \end{tikzpicture}
